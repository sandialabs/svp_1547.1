...
def params(info, group_name=None):
    ...
    info.param(pname('comm'), label='Communications Interface', default='VISA', values=['VISA', 'GPIB' ,'Serial'])
    info.param(pname('visa_address'), label='VISA address', default='GPIB0::1::INSTR',active=pname('comm'), active_value=['VISA'])
    info.param(pname('com_port'), label='COM Port', default='ASRL4',active=pname('comm'), active_value=['Serial'])
...
class DAS(das.DAS):
    ...
    def __init__(self, ts, group_name, points=None, sc_points=None):
        ...
        self.params['com_port'] = self._param_value('com_port')
